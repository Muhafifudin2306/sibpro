...
class GatewayController extends Controller
{
    ...
    public function callback(Request $request)
    {
        ...
        if (!empty($request->phone) && !empty($request->status_code) && !empty($request->gross_amount)) {

            $hashed = hash('sha512', $request->phone.$request->status_code.$request->gross_amount.$serverKey);

            if ($hashed == $request->signature_key) {
                if ($request->transaction_status == 'settlement') {
                    Payment::where("invoice_number",'=',$request->phone)->update([
                        'status' => 'Paid'
                    ]);
                    \Log::info('Pembayaran berhasil. ID Pesanan: ' . $request->phone);
                }
